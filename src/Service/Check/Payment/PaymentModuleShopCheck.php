...
class PaymentModuleShopCheck implements CheckInterface
{
    ...
    {
        return 'Modules de paiement — association boutiques';
    }
    ...
    public function run(): CheckResult
    {
        ...
        // Query 1: Get all modules linked to the paymentOptions hook
        $sqlModules = sprintf(
        ...
        foreach ($modules as $module) {
            ...
            foreach ($shops as $shop) {
                ...
                // Check hook_module for paymentOptions
        ...
        if (empty($issues)) {
            return new CheckResult(
                ...
                label: $this->getLabel(),
                status: 'ok',
                message: sprintf('%d module(s) de paiement vérifiés — aucun problème détecté.', $moduleCount),
                check_query: "SELECT m.name, s.id_shop, s.name as shop_name\nFROM {prefix}module m\nJOIN {prefix}hook_module hm ON hm.id_module = m.id_module\nJOIN {prefix}hook h ON h.id_hook = hm.id_hook AND h.name = 'paymentOptions'\nCROSS JOIN {prefix}shop s ON s.active = 1\nLEFT JOIN {prefix}module_shop ms ON ms.id_module = m.id_module AND ms.id_shop = s.id_shop\nWHERE ms.id_module IS NULL;",
            );
        }

        // Build a summary listing affected modules per shop
        $missingParts = [];
        foreach ($issues as $issue) {
            $missingParts[] = $issue['detail'];
        }
